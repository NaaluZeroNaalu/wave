...
def generate_ncr_report(df, start_date=None, end_date=None):
    with st.spinner("Generating Safety NCR Report with WatsonX..."):
        # Use current date if end_date not provided
        end_date = end_date or datetime.now()
        # Filter for NCRs with "Safety" in description and Days <= 21
        filtered_df = df[
            (df['Description'].str.contains('Safety', case=False, na=False)) &
            (df['Days'] is not None) & 
            (df['Days'] <= 21) & 
            (df['Days'] > 0)
        ].copy()
        ...
        for i in range(0, len(cleaned_data), chunk_size):
            ...
            prompt = (
                ...
                "Task: For Safety NCRs, count ALL records in the provided data by site ('Tower' field) where 'Days' is between 1 and 21 (inclusive) and 'Description' contains the word 'Safety' (case-insensitive). Use the 'Tower' values exactly as they appear in the data (e.g., 'Veridia-Club', 'Veridia-Tower02', 'External Development') and assign each count to the 'Count' key, representing 'No. of NCR to be closed/open within 21 days' only for descriptions mentioning 'Safety'.\n\n"
                            ...
                            try:
                                ...
                                for site, values in chunk_sites.items():
                                    if site not in result["Safety"]["Sites"]:
                                        result["Safety"]["Sites"][site] = {"Count": 0}
...
def generate_consolidated_ncr_excel(combined_result, report_title="Safety: December"):
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        worksheet.write(row, 1, 'No. of NCR to be closed/open within 21 days', header_format)
        
        row = 2
        for site in sorted_sites:
            worksheet.write(row, 0, site, site_format)
            original_key = next((k for k, v in site_mapping.items() if v == site), None)
            value = data.get(original_key, {}).get("Count", 0) if original_key else 0
...
# Streamlit UI
st.title("Asite Safety NCR Reporter")
...
if st.session_state["df"] is not None:
    ...
    st.sidebar.title("📋 Safety NCR Report")
    start_date = st.sidebar.date_input("Start Date", df['Created Date (WET)'].min())
    end_date = st.sidebar.date_input("End Date", df['Created Date (WET)'].max())
    
    if st.sidebar.button("Generate Safety NCR Report"):
        report_title = f"Safety: {end_date.strftime('%B')}"
        ...
        #     data=excel_file,
        #     file_name=f"Safety_NCR_Report_{end_date.strftime('%B')}.xlsx",
