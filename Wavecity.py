...
def GetWaveCity(exceldatas):
    ...
    try:
        datas = pd.read_excel(exceldatas, sheet_name='MSP Progress')
        # st.write(datas.head())
        Ai_answer = generatePrompt(datas.head(), "Wave City Club Start-finish")
        st.write(Ai_answer)
        json_data = json.loads(Ai_answer)
            # st.write(Ai_answer)
        return {"Project":"Wave City Club","Tower Name":"Wave City","Strucutre":json_data[0]['percentage'],"Finishing":"0%"}
