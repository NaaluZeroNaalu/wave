...
def generatePrompt(json_datas):
    body = {
      ...
      Read all data from this table carefully:
        ...
        For each tower, count the exact number of occurrences of 0 and 1 based on the data provided. Ensure the counting process considers all relevant entries without missing any values.
        
        Smaple json foe needed:
        {{"Towername": {{
            "0": total zero count, "1": total one count}}
        }}
...
def CountingProcess3(uploaded_file):
    ...
    # st.dataframe(pour1_subset)
        ...
        average_df.columns = ['Tower Name', 'Average']
        ...
        average_df.insert(0, 'Project', 'Eden')
